...
def generate_orders(config):
    ...
    for index in duplicate_indices:
        ...
        duplicate['order_timestamp'] = duplicate['order_timestamp'] + timedelta(seconds=random.randint(10, 300))
    ...
    for index in late_data:
        ...
        orders[index]['order_timestamp'] = orders[index]['order_timestamp'] - timedelta(minutes=delay_minutes)
        issues_count['late_data'] += 1

    num_missing_id = int(num_records * missing_rate)
    missing_id_indices = random.sample(range(len(orders)), min(num_missing_id, len(orders)))
...
def save_to_csv(orders, output_file):
    ...
    fieldnames = ['order_id', 'customer_id', 'product_id', 'order_timestamp',
    ...
    with open(output_path, 'w', newline='') as csvfile:
        ...
        for order in orders:
            writer.writerow({
                ...
                'order_timestamp': order['order_timestamp'].strftime('%Y-%m-%d %H:%M:%S'),
